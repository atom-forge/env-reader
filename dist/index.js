...
export function asReadonly(value) {
    return value;
}
export class EnvReader {
    ...
    oneOf(key, values, defaultValue) {
        ...
        if (!values.includes(trimmed))
            throw Error(`Env variable type failed: ${key} (oneOf) - expected one of: ${values.join(", ")}`);
        return trimmed;
    }
    /**
    ...
    float(key, defaultValue) {
        ...
        if (isNaN(value))
            throw Error(`Env variable type failed: ${key} (float)`);
        return value;
    }
    /**
    ...
    int(key, defaultValue, radix = 10) {
        ...
        if (isNaN(value))
            throw Error(`Env variable type failed: ${key} (int)`);
        return value;
    }
    /**
    ...
    boolean(key, defaultValue) {
        ...
        if (["1", "yes", "true"].indexOf(envValue.toLowerCase().trim()) != -1)
            return true;
        if (["0", "no", "false"].indexOf(envValue.toLowerCase().trim()) != -1)
            return false;
     ...
     */
    string(key, defaultValue, trim = true) {
        let envValue = process.env[key];
        if (envValue === undefined) {
            if (defaultValue !== undefined)
                return defaultValue;
        ...
        if (envValue === undefined) {
            if (defaultValue !== undefined) {
                try {
                    ...
                    throw Error(`Env variable default is not a valid url: ${key} - ${defaultValue}`);
                }
            }
            throw Error(`Missing Env variable (url): ${key}`);
        }
        try {
            return new URL(envValue.trim());
        }
        catch {
            throw Error(`Env variable type failed: ${key} (url)`);
        }
    }
    /**
    ...
    regex(key, defaultValue) {
        ...
        try {
            return new RegExp(envValue.trim());
        }
        catch {
            throw Error(`Env variable type failed: ${key} (regex)`);
        }
    }
    /**
     ...
     */
    list(key, parser, defaultValue, separator = ",") {
        let envValue = process.env[key];
        if (envValue === undefined) {
            if (defaultValue !== undefined)
                return defaultValue;
        ...
        return envValue
            ...
            try {
                ...
                throw Error(`Env variable type failed: ${key}[${i}] - ${v}`);
            }
        });
    }
    /**
    ...
    file(key, defaultValue, onMissing = true, stayInProject = true) {
        ...
        if (envValue === undefined) {
            if (defaultValue !== undefined)
                ...
                throw Error(`Missing Env variable (file): ${key}`);
        }
        ...
        if (!fs.existsSync(resolved) || !fs.statSync(resolved).isFile()) {
            if (onMissing === true)
                throw Error(`Env variable path is not a file: ${key} - ${resolved}`);
    ...
    dir(key, defaultValue, onMissing = true, stayInProject = true) {
        ...
        if (envValue === undefined) {
            if (defaultValue !== undefined)
                ...
                throw Error(`Missing Env variable (dir): ${key}`);
        }
        const resolved = stayInProject
            ? path.resolve(this.projectRoot, envValue.trim())
            : path.resolve(envValue.trim());
        ...
        if (!fs.existsSync(resolved) || !fs.statSync(resolved).isDirectory()) {
            if (onMissing === true)
                throw Error(`Env variable path is not a directory: ${key} - ${resolved}`);
            if (typeof onMissing === "function")
                onMissing(resolved);
        }
        return resolved;
